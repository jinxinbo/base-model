...
Data Temp_WOE1;
 ...
 by &IVVar &DVVar;
 if first.&IVVar=last.&IVVar then do;
	if &DVVar=0 then do;
		C0=Count;
	    C0T=C0T+C0;
		C1=0;
 ...
 else if first.&IVVar then do;
      C0=Count;
	  C0T=C0T+C0;
  end;
  else if last.&IVVar then do;
       C1=Count;
	   C1T=C1T+C1;
  end;
...
Data &WOEDs;
 set Temp_WOE1;
  ...
  Else WOE=.;
  if GoodDist>0 then do;
      if BadDist>0 then WOE=log(BadDist/GoodDist);
	    else woe=-9;
      end;
   else woe=9;
  keep &IVVar WOE;
...
dataset */
proc sql noprint;
	create table &dsout(drop=&IvVar) as 
	select a.* , b.woe from &dsin a, &woeds b where a.&IvVar=b.&IvVar; 
quit;
data &dsout;
   set &dsout;
   rename woe=&IvVar;
